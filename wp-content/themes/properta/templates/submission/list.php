...
<?php if ( ! empty( $create_page_id ) ) : ?>
	<?php if ( Realia_Packages::is_allowed_to_add_submission( get_current_user_id() ) ) :   ?>
		<a href="<?php echo get_permalink( $create_page_id ); ?>" class="property-create"><?php echo __( 'Create Property', 'properta' ); ?></a>
	<?php endif; ?>
<?php endif; ?>
	...
	<table class="property-table">
		...
		<?php while ( have_posts() ) : the_post(); ?>
			<tr>
					...
					<div class="property-table-actions-inner">
						...
						<?php if ( ! empty( $payment_page_id ) ) : ?>
								...
								<?php if ( ! empty( $enable_sticky ) ) : ?>
									...
									<?php if ( ! empty( $price ) ) : ?>
										<form method="post" action="<?php echo get_permalink( $payment_page_id ); ?>">
											...
											<button type="submit">
												<?php echo __( 'Make TOP', 'properta' ); ?> <span class="label label-primary"><?php echo Realia_Price::format_price( $price ); ?></span>
											</button>
										</form>
									<?php endif; ?>
								<?php endif; ?>
								...
								<?php if ( ! empty( $enable_featured ) ) : ?>
									...
									<?php if ( ! empty( $price ) ) : ?>
										<form method="post" action="<?php echo get_permalink( $payment_page_id ); ?>">
											...
											<button type="submit">
												<?php echo __( 'Make featured', 'properta' ); ?> <span class="label label-primary"><?php echo Realia_Price::format_price( $price ); ?></span>
											</button>
										</form>
									<?php endif; ?>
								<?php endif; ?>
							...
							<?php if ( 'publish' == $property_status ) : ?>
								<button class="disabled">
									<?php echo __( 'Published', 'properta' ); ?>
								</button>
							<?php else : ?>
								<!-- PAY PER POST -->
								<?php if ( 'pay-per-post' == $submission_type ) : ?>
									...
									<?php if ( ! empty( $price ) ) : ?>
										<form method="post" action="<?php echo get_permalink( $payment_page_id ); ?>">
											...
											<button type="submit">
												<?php echo __( 'Publish', 'properta' ); ?> <span class="label label-primary"><?php echo Realia_Price::format_price( $price ); ?></span>
											</button>
										</form>
									<?php endif; ?>
				...
				<td class="min-width nowrap property-table-edit-remove">
					...
					<?php if ( ! empty( $edit_page_id ) ) : ?>
						<a href="<?php echo get_permalink( $edit_page_id ); ?>?id=<?php the_ID(); ?>" class="property-table-action">
							<span><?php echo __( 'Edit', 'properta' ); ?></span>
						</a>
					<?php endif; ?>
					...
					<?php if ( ! empty( $remove_page_id ) ) : ?>
						<a href="<?php echo get_permalink( $remove_page_id ); ?>?id=<?php the_ID(); ?>" class="property-table-action property-button-delete">
                            <span><?php echo __( 'Remove', 'properta' ); ?></span>
						</a>
					<?php endif; ?>
				</td>
			</tr>
...
<?php else : ?>
	<div class="alert alert-warning">
		<p><?php echo __( 'You don\'t have any properties, yet. Start by creating new one.', 'properta' )?></p>
	</div>
<?php endif; ?>

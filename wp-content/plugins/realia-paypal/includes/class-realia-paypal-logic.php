...
require REALIA_PAYPAL_DIR . 'libraries/PayPal-PHP-SDK/vendor/autoload.php';
...
/**
 * Class Realia_PayPal_Logic
 *
 * @class Realia_PayPal_Logic
 * @package Realia_PayPal/Classes
...
class Realia_PayPal_Logic {
	...
	public static function payment_gateways( $gateways ) {
		if (  Realia_Utilities::is_paypal_enabled() ) {
			if ( get_theme_mod( 'realia_paypal_credit_card', false ) == '1' ) {
				$gateways[] = array(
					'id'      => 'paypal-credit-card',
					'title'   => __( 'PayPal Credit Card', 'realia-credit-card' ),
					'proceed' => true,
					'content' => Realia_Template_Loader::load( 'paypal/credit-card-form', array(), REALIA_PAYPAL_DIR ),
				);
			}

			$gateways[] = array(
				'id'      => 'paypal-account',
				'title'   => __( 'PayPal Acccount', 'realia-paypal' ),
				'proceed' => true
			);
		}
        ...
        if ( ! empty( $_GET['success'] ) && ! empty( $_GET['user_id'] ) && ! empty( $_GET['payment_type'] ) && ! empty( $_GET['object_id'] ) ) {
            if ( $_GET['success'] == 'true' ) {
                ...
                $_SESSION['messages'][] = array(
                    'success', __( 'Payment has been successful.', 'realia' )
                );
            } else {
                $_SESSION['messages'][] = array(
